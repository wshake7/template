# Backend Repo Models 编写与维护

## 场景与目标
- 适用场景：修改 `backend/admin/services/repo/models` 下实体模型、字段、关联关系、表结构时
- 目标：保持模型定义、自动迁移、`gorm/gen` 查询代码三者一致，降低回归风险

## 目录/文件位置
- 模型目录：`backend/admin/services/repo/models/`
- 模型注册入口：`backend/admin/services/repo/models/models.go`
- 仓储初始化：`backend/admin/services/repo/repo.go`
- 查询代码生成目录：`backend/admin/services/repo/query/`
- 生成脚本：`backend/admin/cmd/script/repo.go`

## 当前模型组织方式（按现状）
1. 每个模型文件通过 `init()` 执行 `Models = append(Models, &Xxx{})`
2. `repo.New()` 中通过 `gormCrud.WithAutoMigrate(models.Models...)` 执行自动迁移
3. `repo.New()` / `cmd/script/repo.go` 可触发 `gen` 生成 `query/*.gen.go`
...
3. 若涉及迁移：确认 `config.Repo.IsAutoMigrate` 或手工迁移策略
...
4. 当前 `sys_login_log.go` 内容为空，若计划启用请先补完整模型并注册
