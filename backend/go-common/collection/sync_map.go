...
// SyncMap is an implementation of a concurrent hash-trie. The implementation
...
//
// The zero SyncMap is empty and ready to use.
...
func (ht *SyncMap[K, V]) Load(key K) (value V, ok bool) {
	ht.init()
	...
	panic("internal/sync.SyncMap: ran out of hash bits while iterating")
}

// LoadOrStore returns the existing value for the key if present.
// Otherwise, it stores and returns the given value.
// The loaded result is true if the value was loaded, false if stored.
func (ht *SyncMap[K, V]) LoadOrStore(key K, value V) (result V, loaded bool) {
	ht.init()
	hash := ht.keyHash(key, ht.seed)
	var i *indirect[K, V]
	var hashShift uint
...
func (ht *SyncMap[K, V]) expand(oldEntry, newEntry *entry[K, V], newHash uint64, hashShift uint, parent *indirect[K, V]) *node[K, V] {
	...
	for {
		if hashShift == 0 {
			panic("internal/sync.SyncMap: ran out of hash bits while inserting (incorrect use of unsafe or cgo, or data race?)")
		}
...
func (ht *SyncMap[K, V]) Store(key K, new V) {
	_, _ = ht.Swap(key, new)
}

// Swap swaps the value for a key and returns the previous value if any.
// The loaded result reports whether the key was present.
func (ht *SyncMap[K, V]) Swap(key K, new V) (previous V, loaded bool) {
	ht.init()
	hash := ht.keyHash(key, ht.seed)
	var i *indirect[K, V]
	var hashShift uint
...
func (ht *SyncMap[K, V]) CompareAndSwap(key K, old, new V) (swapped bool) {
	ht.init()
	if ht.valEqual == nil {
...
// The loaded result reports whether the key was present.
func (ht *SyncMap[K, V]) LoadAndDelete(key K) (value V, loaded bool) {
	ht.init()
	hash := ht.keyHash(key, ht.seed)
...
func (ht *SyncMap[K, V]) Delete(key K) {
...
func (ht *SyncMap[K, V]) CompareAndDelete(key K, old V) (deleted bool) {
	ht.init()
	if ht.valEqual == nil {
...
func (ht *SyncMap[K, V]) find(key K, hash uint64, valEqual equalFunc[V], value V) (i *indirect[K, V], hashShift uint, slot *atomic.Pointer[node[K, V]], n *node[K, V]) {
	for {
...
// SyncMap's contents: no key will be visited more than once, but if the value
...
// method on the SyncMap.
func (ht *SyncMap[K, V]) All() func(yield func(K, V) bool) {
	ht.init()
...
func (ht *SyncMap[K, V]) Range(yield func(K, V) bool) {
	ht.init()
	ht.iter(ht.root.Load(), yield)
}

func (ht *SyncMap[K, V]) iter(i *indirect[K, V], yield func(key K, value V) bool) bool {
...
// Clear deletes all the entries, resulting in an empty SyncMap.
func (ht *SyncMap[K, V]) Clear() {
	ht.init()
...
// This operation requires iterating through all entries, so it has O(n) time complexity.
func (ht *SyncMap[K, V]) Len() int {
	ht.init()
	return ht.count(ht.root.Load())
}

// IsEmpty returns true if the map contains no entries.
// This is more efficient than checking Len() == 0 as it can short-circuit
// on the first found entry.
func (ht *SyncMap[K, V]) IsEmpty() bool {
	ht.init()
	return ht.isEmpty(ht.root.Load())
}

// count recursively counts all entries in the hash-trie starting from node i.
func (ht *SyncMap[K, V]) count(i *indirect[K, V]) int {
	total := 0
	for j := range i.children {
		n := i.children[j].Load()
		if n == nil {
		...
		if !n.isEntry {
			total += ht.count(n.indirect())
			continue
		}
		// Count all entries in the overflow chain
		e := n.entry()
		for e != nil {
			total++
			e = e.overflow.Load()
...
// isEmpty checks if the hash-trie is empty starting from node i.
// It returns true immediately upon finding the first entry.
func (ht *SyncMap[K, V]) isEmpty(i *indirect[K, V]) bool {
	for j := range i.children {
		n := i.children[j].Load()
		if n == nil {
			continue
...
func (ht *SyncMap[K, V]) ToMap() map[K]V {
	result := map[K]V{}
	ht.All()(func(e K, v V) bool {
		result[e] = v
		return true
	})
	return result
